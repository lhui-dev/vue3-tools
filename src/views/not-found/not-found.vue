<template>
  <div class="http404-container">
    <div class="http404-wrapper">
      ...
      <div class="bullshit">
        <div class="bullshit__oops">{{ messageConfig.oopsMessage }}~</div>
        <div class="bullshit__headline">{{ messageConfig.handLineMessage }}</div>
        <div class="bullshit__info">{{ messageConfig.infoMessage }}</div>
        <router-link to="/" class="bullshit__return-home">{{
          messageConfig.toReturnMessage
        }}</router-link>
        <router-link to="/login" class="bullshit__return-home">{{
          messageConfig.toLoginMessage
        }}</router-link>
      </div>
    </div>
  </div>
...
const messageConfig = reactive({
  oopsMessage: '页面找不到了',
  handLineMessage: '该页面您无法进入...',
  infoMessage: '请检查您输入的URL是否正确，或者点击下面的按钮返回首页。',
  toReturnMessage: '返回首页',
  toLoginMessage: '去登录'
});
</script>
